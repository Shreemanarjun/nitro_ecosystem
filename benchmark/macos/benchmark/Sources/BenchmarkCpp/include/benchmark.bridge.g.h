// Generated by Nitrogen Modules. Do not edit.
// nitro_generator: 0.7.7
// Generated from: benchmark.native.dart
// spec-sha256: 6b83b8b117e1c2793f4dd49860a561741b9c0e71c53c8b0df6c1e0de44077373
#pragma once

#include <stdint.h>
#include <stdbool.h>
#include <stdlib.h>

#ifndef NITRO_EXPORT
#  if defined(_WIN32)
#    define NITRO_EXPORT __declspec(dllexport)
#  else
#    define NITRO_EXPORT __attribute__((visibility("default"))) __attribute__((used))
#  endif
#endif

#ifndef NITRO_ERROR_DEFINED
#define NITRO_ERROR_DEFINED
typedef struct {
  int8_t hasError;
  const char* name;
  const char* message;
  const char* code;
  const char* stackTrace;
} NitroError;
#endif

#ifndef NITRO_OPT_DEFINED
#define NITRO_OPT_DEFINED
#pragma pack(push, 1)
typedef struct { uint8_t hasValue; int64_t  value; } NitroOptInt64;
typedef struct { uint8_t hasValue; double   value; } NitroOptFloat64;
typedef struct { uint8_t hasValue; uint8_t  value; } NitroOptBool;
#pragma pack(pop)
#endif

#ifdef __cplusplus
extern "C" {
#endif

NITRO_EXPORT uint32_t benchmark_nitro_abi_version(void);
NITRO_EXPORT const char* benchmark_nitro_bridge_checksum(void);
NITRO_EXPORT intptr_t benchmark_init_dart_api_dl(void* data);
NITRO_EXPORT NitroError* benchmark_get_error(void);
NITRO_EXPORT void benchmark_clear_error(void);
NITRO_EXPORT int64_t benchmark_create_instance(const char* key);
NITRO_EXPORT void benchmark_destroy_instance(int64_t instanceId);
NITRO_EXPORT void benchmark_nitro_free(void* ptr);
NITRO_EXPORT void* benchmark_nitro_alloc(size_t size);
struct _Dart_CObject;
NITRO_EXPORT bool benchmark_nitro_post(int64_t port, struct _Dart_CObject* obj);
NITRO_EXPORT int64_t benchmark_nitro_bind(int64_t batchPort);
NITRO_EXPORT void benchmark_nitro_ack(int64_t batchPort);
#define Dart_PostCObject_DL(port, obj) benchmark_nitro_post((port), (obj))



// Methods
NITRO_EXPORT double benchmark_add(int64_t instanceId, double a, double b, NitroError* _nitro_err);
NITRO_EXPORT double benchmark_add_fast(int64_t instanceId, double a, double b, NitroError* _nitro_err);
NITRO_EXPORT const char* benchmark_get_greeting(int64_t instanceId, const char* name, NitroError* _nitro_err);
NITRO_EXPORT int64_t benchmark_hash_buffer(int64_t instanceId, uint8_t* data, size_t data_length, int64_t rounds, NitroError* _nitro_err);
NITRO_EXPORT int64_t benchmark_sieve_primes(int64_t instanceId, int64_t limit, NitroError* _nitro_err);
NITRO_EXPORT int64_t benchmark_send_large_buffer(int64_t instanceId, uint8_t* buffer, size_t buffer_length, NitroError* _nitro_err);

#ifdef __cplusplus
}
#endif
