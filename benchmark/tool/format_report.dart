...
void main(List<String> args) {
  ...
  final out = StringBuffer()
    ...
    ..writeln()
    ..write(
      '| Case | Median | Overhead vs raw FFI | vs MethodChannel '
  ...
  for (final entry in cases.entries) {
    ...
    out
      ..write(
        '| ${c['label']} | ${median.toStringAsFixed(3)} µs '
  ...
  if (throughputRows.isNotEmpty) {
    out
      ...
      ..writeln('|---|---|---|---|');
    final channelMb =
        ((cases['channel_buffer'] as Map<String, dynamic>?)?['mbPerSec']
                as num?)
            ?.toDouble();
  ...
  final pinnedMb =
      ((cases['nitro_buffer_pinned'] as Map<String, dynamic>?)?['mbPerSec']
              as num?)
          ?.toDouble();
  final copyMb =
  ...
  if (baseCases != null) {
    ...
    for (final entry in cases.entries) {
      ...
      if (pct.abs() >= 10) {
        drifts.add(
          '`${entry.key}` ${pct > 0 ? 'slower' : 'faster'} by '
    ...
    if (drifts.isEmpty) {
      out.writeln(
        'All latency cases within ±10% of the recorded '
        '$platform baseline.',
      );
    } else {
      out.writeln(
        'Cases drifting ≥10% vs the recorded baseline '
  ...
  if (compareDir != null && Directory(compareDir).existsSync()) {
    ...
    if (others.length > 1) {
      ...
      final caseIds = <String>{for (final c in others.values) ...c.keys}.where((
        id,
      ) {
        // Latency cases only — throughput MB/s columns don't align well.
        return others.values.any(
          (c) => (c[id] as Map<String, dynamic>?)?['kind'] == 'latency',
        );
      });
      out
        ..writeln()
      ...
      out.writeln();
      out.writeln(
        '_Numbers from the most recent archived run per platform '
