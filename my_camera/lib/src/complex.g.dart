...
class _ComplexModuleImpl extends ComplexModule {
  ...
  late final Pointer<NitroErrorFfi> Function() _getErrorPtr = _dylib.lookupFunction<Pointer<NitroErrorFfi> Function(), Pointer<NitroErrorFfi> Function()>('complex_get_error');
  late final void Function() _clearErrorPtr = _dylib.lookupFunction<Void Function(), void Function()>('complex_clear_error');
  late final Pointer<NativeFunction<Pointer<NitroErrorFfi> Function()>> _getErrorNativePtr = _dylib.lookup('complex_get_error');
  late final Pointer<NativeFunction<Void Function()>> _clearErrorNativePtr = _dylib.lookup('complex_clear_error');
  ...
  Future<String> fetchMetadata(String url) async {
    ...
    final arena = Arena();
    try {
      final rawPtr = await NitroRuntime.callAsync<Pointer<Utf8>>(_fetchMetadataPtr, [url.toNativeUtf8(allocator: arena)], getError: _getErrorNativePtr, clearError: _clearErrorNativePtr);
  ...
  Future<Packet> generatePacket(int type) async {
    checkDisposed();
    final rawPtr = await NitroRuntime.callAsync<Pointer<Void>>(_generatePacketPtr, [type], getError: _getErrorNativePtr, clearError: _clearErrorNativePtr);
  ...
  @override
  set config(String value) { checkDisposed(); withArena((arena) { _setConfigPtr(value.toNativeUtf8(allocator: arena)); NitroRuntime.checkError(_getErrorPtr, _clearErrorPtr); }); }
