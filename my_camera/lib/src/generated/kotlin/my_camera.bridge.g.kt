...
data class CameraFrame(val data: java.nio.ByteBuffer, val width: Long, val height: Long, val stride: Long, val timestampNs: Long) {
    companion object {
        @JvmStatic fun decodeFrom(buf: java.nio.ByteBuffer): CameraFrame {
            ...
            val height = buf.long
            val stride = buf.long
            val timestampNs = buf.long
            return CameraFrame(data, width, height, stride, timestampNs)
        }
    ...
    fun writeFieldsTo(out: java.io.ByteArrayOutputStream, buf: java.nio.ByteBuffer) {
        ...
        writeInt(height)
        writeInt(stride)
        writeInt(timestampNs)
    }

    fun encode(): ByteArray {
        val out = java.io.ByteArrayOutputStream(68)
...
data class CameraDevice(val id: String, val name: String, val resolutions: List<Resolution>, val isFrontFacing: Boolean) {
    ...
    fun writeFieldsTo(out: java.io.ByteArrayOutputStream, buf: java.nio.ByteBuffer) {
        ...
        resolutions.forEach { e -> e.writeFieldsTo(out, buf) }
