...
String _generateCppRecords(BridgeSpec spec) {
  ...
  // NitroOpt* types are already defined in the bridge.g.h C header as
  // anonymous-struct typedefs (e.g. `typedef struct { ... } NitroOptInt64`).
  // Emitting `struct NitroOptInt64;` here would conflict with that typedef.
  // Skip them — the bridge.g.h #include is the single source of truth.
  final cppRecords = localRecords.where((r) => !_nitroLibraryRecordTypes.contains(r.name)).toList();
  if (cppRecords.isEmpty) return '';
