...
class KotlinGenerator {
  static String generate(BridgeSpec spec) {
    ...
    if (spec.entryPoints.isNotEmpty) {
      ...
      writer.line('    private val _bgEngines = java.util.concurrent.ConcurrentHashMap<Long, io.flutter.embedding.engine.FlutterEngine>()');
      writer.line('    private val _bgCallbacks = java.util.concurrent.ConcurrentHashMap<Long, (Long, String?) -> Unit>()');
      writer.line('    private val _bgFinished = java.util.concurrent.ConcurrentHashMap<Long, String>()');
      writer.line('    private val _bgMain = android.os.Handler(android.os.Looper.getMainLooper())');
      writer.line('    private var _bgGroup: io.flutter.embedding.engine.FlutterEngineGroup? = null');
      writer.line('    private const val _bgNoError = "\\u0000ok"');
      writer.line('    @JvmStatic external fun nitroBgFail(jobId: Long, error: String)');
      writer.blankLine();
      writer.line('    /// Called by the C bridge (any thread) for a job: starts a headless engine at');
      writer.line('    /// the generated entry wrapper, passing the job id as the entrypoint argument');
      writer.line('    /// so that engine takes exactly that job. Engines are spawned from one');
      writer.line('    /// FlutterEngineGroup — they share the VM, snapshot and font/GPU context, so');
      writer.line('    /// N concurrent jobs cost N isolates, not N engines from scratch.');
      writer.line('    @Keep @JvmStatic fun nitroBgStart(entry: String, jobId: Long) {');
      writer.line('        _bgMain.post {');
      writer.line('            try {');
      writer.line('                val loader = io.flutter.FlutterInjector.instance().flutterLoader()');
      writer.line('                if (!loader.initialized()) {');
      writer.line('                    loader.startInitialization(applicationContext)');
      writer.line('                    loader.ensureInitializationComplete(applicationContext, null)');
      writer.line('                }');
      writer.line('                val group = _bgGroup ?: io.flutter.embedding.engine.FlutterEngineGroup(applicationContext).also { _bgGroup = it }');
      writer.line('                val options = io.flutter.embedding.engine.FlutterEngineGroup.Options(applicationContext)');
      writer.line('                    .setDartEntrypoint(io.flutter.embedding.engine.dart.DartExecutor.DartEntrypoint(');
      writer.line("                        loader.findAppBundlePath(), \"${spec.entryPointLibraryUri}\", \"nitroEntry_\" + entry))");
      writer.line('                    .setDartEntrypointArgs(listOf(jobId.toString()))');
      writer.line('                _bgEngines[jobId] = group.createAndRunEngine(options)');
      writer.line('            } catch (t: Throwable) {');
      writer.line('                _bgEngines.remove(jobId)?.destroy()');
      writer.line('                nitroBgFail(jobId, "background engine start failed: " + t)');
      writer.line('            }');
      writer.line('        }');
      writer.line('    }');
      writer.blankLine();
      writer.line('    @JvmStatic external fun nitroBgRunString(entry: String, text: String): Long');
      writer.blankLine();
      writer.line('    /// Native-initiated background job — a WorkManager worker, a BroadcastReceiver,');
      writer.line('    /// a push handler — with no Flutter UI running. The entry must take a single');
      writer.line('    /// String; whatever it persists is the result. Returns the job id, or -1 when');
      writer.line('    /// no engine could be started. [onDone] runs on the main thread once the job');
      writer.line('    /// finished: `error` is null on success, else the thrown error\'s text (also');
      writer.line('    /// logged under the "Nitro" tag).');
      writer.line('    @JvmStatic @JvmOverloads fun runInBackground(context: Context, entry: String, text: String, onDone: ((jobId: Long, error: String?) -> Unit)? = null): Long {');
      writer.line('        if (!::applicationContext.isInitialized) applicationContext = context.applicationContext');
      writer.line("        System.loadLibrary(\"${spec.lib.replaceAll('-', '_')}\")");
      writer.line('        initialize(this::class.java)');
      writer.line('        val id = nitroBgRunString(entry, text)');
      writer.line('        if (id < 0 || onDone == null) return id');
      writer.line('        _bgCallbacks[id] = onDone');
      writer.line('        // The job may already have finished (fast entry): deliver now.');
      writer.line('        val early = _bgFinished.remove(id)');
      writer.line('        if (early != null && _bgCallbacks.remove(id) != null) _bgMain.post { onDone(id, if (early == _bgNoError) null else early) }');
      writer.line('        return id');
      writer.line('    }');
      writer.blankLine();
      writer.line('    /// Blocking form for workers (WorkManager, JobIntentService): returns once the');
      writer.line('    /// job finished, with `null` on success or the error text on failure / timeout /');
      writer.line('    /// no host. Must not be called on the main thread — the engine starts there.');
      writer.line('    @JvmStatic @JvmOverloads fun runInBackgroundAndWait(context: Context, entry: String, text: String, timeoutMs: Long = 30_000L): String? {');
      writer.line('        check(android.os.Looper.myLooper() != android.os.Looper.getMainLooper()) { "runInBackgroundAndWait must not be called on the main thread" }');
      writer.line('        val latch = java.util.concurrent.CountDownLatch(1)');
      writer.line('        var result: String? = null');
      writer.line('        val id = runInBackground(context, entry, text) { _, error -> result = error; latch.countDown() }');
      writer.line('        if (id < 0) return "no background host: could not start an engine for " + entry');
      writer.line('        return if (latch.await(timeoutMs, java.util.concurrent.TimeUnit.MILLISECONDS)) result else "timeout after " + timeoutMs + "ms waiting for " + entry');
      writer.line('    }');
      writer.blankLine();
      writer.line('    /// Headless engines currently alive for this library\'s jobs.');
      writer.line('    @JvmStatic fun activeBackgroundEngines(): Int = _bgEngines.size');
      writer.blankLine();
      writer.line('    /// The job finished (result already posted, or it failed with [error]): tear');
      writer.line('    /// the engine down and notify a native completion callback, if any.');
      writer.line('    @Keep @JvmStatic fun nitroBgDone(jobId: Long, error: String?) {');
      writer.line('        if (error != null) android.util.Log.e("Nitro", "background job " + jobId + " failed: " + error)');
      writer.line('        _bgMain.post {');
      writer.line('            _bgEngines.remove(jobId)?.destroy()');
      writer.line('            val cb = _bgCallbacks.remove(jobId)');
      writer.line('            if (cb != null) {');
      writer.line('                cb(jobId, error)');
      writer.line('            } else {');
      writer.line('                if (_bgFinished.size > 256) _bgFinished.clear()');
      writer.line('                _bgFinished[jobId] = error ?: _bgNoError');
      writer.line('            }');
      writer.line('        }');
      writer.line('    }');
    }
