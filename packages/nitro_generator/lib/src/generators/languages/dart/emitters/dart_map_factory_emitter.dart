...
void _emitMapAndFactory(CodeWriter writer, BridgeSpec spec) {
  ...
  for (final func in spec.functions) {
    ...
    for (final p in func.params) {
      _collectMapValueTypes(p.type, mapValueTypes);
    }
  }
  ...
  // Also collect int-key map types (Gap #3: Map<int,V> / Map<Enum,V>).
  final intKeyMapTypes = <({String keyType, String valueType})>{};
  for (final func in spec.functions) {
    _collectIntKeyMapTypes(func.returnType, intKeyMapTypes, spec);
    for (final p in func.params) {
      _collectIntKeyMapTypes(p.type, intKeyMapTypes, spec);
    }
  }
  for (final prop in spec.properties) {
    _collectIntKeyMapTypes(prop.type, intKeyMapTypes, spec);
  }

  if (mapValueTypes.isNotEmpty || intKeyMapTypes.isNotEmpty) {
  ...
  if (intKeyMapTypes.isNotEmpty) {
    writer.line('// Int-key map encode/decode helpers (Gap #3)');
    writer.line('// Wire: [4B payload_len][4B count][entries: [N bytes key][value_bytes]]');
    writer.blankLine();
    for (final m in intKeyMapTypes) {
      _emitIntKeyMapBinaryHelpers(writer, m.keyType, m.valueType, spec);
    }
  }
