...
class DartFfiGenerator {
  static String generate(BridgeSpec spec) {
    ...
    // Emit Map<String, double> NaN/Infinity safe helpers if needed.
    if (_hasDoubleMapType(spec)) {
      writer.line('// Map<String, double> NaN/Infinity safe helpers (generated by Nitrogen)');
      writer.line('Map<String, double> _nitroDecodeDoubleMap(Map<String, dynamic> m) =>');
      writer.line("    m.map((k, v) => MapEntry(k, v == '__NaN__' ? double.nan : v == '__Inf__' ? double.infinity : v == '__NInf__' ? double.negativeInfinity : (v as num).toDouble()));");
      writer.line('String _nitroEncodeDoubleMap(Map<String, double> m) =>');
      writer.line("    jsonEncode(m.map((k, v) => MapEntry(k, v.isNaN ? '__NaN__' : v.isInfinite ? (v > 0 ? '__Inf__' : '__NInf__') : v)));");
      writer.blankLine();
    }
  ...
  static bool _hasDoubleMapType(BridgeSpec spec) {
    bool isDoubleMap(BridgeType t) {
      if (!t.isMap) return false;
      final m = RegExp(r'^Map<String,\s*(.+)>$').firstMatch(t.name);
      return m?.group(1)?.trim() == 'double';
    }
    for (final func in spec.functions) {
      if (isDoubleMap(func.returnType)) return true;
      for (final p in func.params) {
        if (isDoubleMap(p.type)) return true;
      }
    }
    for (final prop in spec.properties) {
      if (isDoubleMap(prop.type)) return true;
    }
    return false;
  }
  ...
  static String _decodeRecordExpr(BridgeType type, String ptrVar) {
    if (type.isMap) {
      ...
      if (valueType == 'double') {
        // Special case: NaN/Infinity cannot survive JSON round-trip.
        // Sentinels '__NaN__' / '__Inf__' / '__NInf__' are reversed here.
        return '_nitroDecodeDoubleMap((jsonDecode($ptrVar.toDartString()) as Map<String, dynamic>))';
      }
  ...
  static String _encodeRecordParam(BridgeType type, String varName, String allocator) {
    if (type.isMap) {
      // Map<String, double>: NaN/Infinity cannot be JSON-encoded. Use sentinel helper.
      final mapMatch = RegExp(r'^Map<String,\s*(.+)>$').firstMatch(type.name);
      final valueType = mapMatch?.group(1)?.trim();
      if (valueType == 'double') {
        return '_nitroEncodeDoubleMap($varName).toNativeUtf8(allocator: $allocator)';
      }
