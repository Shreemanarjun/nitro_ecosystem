...
void main() {
  group('<lib>_nitro_free export — Windows CoTaskMemFree heap-corruption fix', () {
    ...
    test('string return never uses toDartStringWithFree (Windows heap safety)', () {
      ...
      // that killed the Windows CI job at its 15th test. Sync returns now
      // borrow a per-thread buffer (no free at all), and any owned string is
      // still released through the module's `<lib>_nitro_free` export.
      final out = DartFfiGenerator.generate(_spec());
