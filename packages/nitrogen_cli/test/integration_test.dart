...
bool _isRepoRoot(String dir) {
  ...
  return pubspec.existsSync() && pubspec.readAsStringSync().contains('workspace:') && Directory(p.join(dir, 'test_projects', 'testing_project')).existsSync();
}
...
void main() {
  ...
  group(
    ...
    skip: _fixture.existsSync() && File(p.join(_fixture.path, 'lib', 'src', 'testing_project.g.dart')).existsSync()
        ? null
        : 'generated output not found — run `nitrogen generate` first',
    () {
  ...
  group(
    'nitrogen generate — testing_project spec (Swift iOS / Kotlin Android)',
    ...
    () {
      ...
      test('Dart binding: add() is a sync call returning double', () {
        ...
        expect(content, isNot(contains('Future<double>')), reason: 'add is sync — must not be wrapped in Future');
      });
      ...
      test('C bridge header: getGreeting() has a dart_port int64 param for async', () {
        final content = File(p.join(_fixture.path, 'lib', 'src', 'generated', 'cpp', 'testing_project.bridge.g.h')).readAsStringSync();
        expect(content, contains('dart_port'), reason: 'async methods post result via Dart port — int64_t dart_port param required');
      });
  ...
  group(
    'nitrogen generate — testing_cpp spec (NativeImpl.cpp all platforms)',
    ...
    () {
      ...
      test('C bridge header: tryDivide uses NitroOptInt64 (nullable int64 return)', () {
        ...
        expect(content, contains('NitroOptInt64'), reason: 'int? return must use packed optional struct, not sentinel value');
      });

      test('C bridge header: isEven return is uint8_t / bool-sized', () {
        final content = File(p.join(_fixture.path, 'lib', 'src', 'generated', 'cpp', 'testing_cpp.bridge.g.h')).readAsStringSync();
        expect(content, matches(RegExp(r'(uint8_t|int8_t|bool)\s+testing_cpp_is_even')), reason: 'bool methods should use a byte-sized C type, not int64_t');
      });
      ...
      test('impl.g.cpp: one-time editable starter with HybridTestingCpp methods', () {
        ...
        expect(content, isNot(contains('Generated by Nitrogen Modules. Do not edit.')), reason: 'impl.g.cpp must NOT say "do not edit" — it is the user-editable starter');
      });
      ...
      test('Swift bridge: no @_cdecl stubs (cpp-only path uses C functions directly)', () {
        ...
        expect(content, isNot(contains('@_cdecl("')), reason: 'NativeImpl.cpp Swift bridge must not emit @_cdecl stubs');
      });
      ...
      test('Kotlin bridge: tryDivide returns Long? (nullable Long)', () {
        final content = File(p.join(_fixture.path, 'lib', 'src', 'generated', 'kotlin', 'testing_cpp.bridge.g.kt')).readAsStringSync();
        expect(content, matches(RegExp(r'fun tryDivide\([^)]*\)\s*:\s*Long\?')), reason: 'int? maps to nullable Long in Kotlin');
      });
      ...
      test('Dart binding: tryDivide returns int? (nullable)', () {
        final content = File(p.join(_fixture.path, 'lib', 'src', 'testing_cpp.g.dart')).readAsStringSync();
        expect(content, contains('int? tryDivide('), reason: 'tryDivide declared int? in the native spec must stay nullable in Dart');
      });
  ...
  group(
    'nitrogen generate — testing_mixed spec (Swift iOS / Kotlin Android / C++ macOS)',
    skip: _fixture.existsSync() && File(p.join(_fixture.path, 'lib', 'src', 'generated', 'swift', 'testing_mixed.bridge.g.swift')).existsSync()
        ? null
    ...
    () {
      ...
      test('C bridge header: optionalFlag uses NitroOptBool return', () {
        ...
        expect(content, contains('NitroOptBool'), reason: 'bool? return must use NitroOptBool packed struct');
      });

      test('C bridge header: optionalValue uses NitroOptFloat64 return', () {
        final content = File(p.join(_fixture.path, 'lib', 'src', 'generated', 'cpp', 'testing_mixed.bridge.g.h')).readAsStringSync();
        expect(content, contains('NitroOptFloat64'), reason: 'double? return must use NitroOptFloat64 packed struct');
      });
...
''');
  ...
  group('multi-spec generate — testing_cpp (NativeImpl.cpp all platforms)', skip: multiSpecSkip(), () {
    ...
    test('testing_cpp.bridge.g.swift has no @_cdecl function stubs (cpp-only path)', () {
      ...
      expect(content, isNot(contains('@_cdecl("')), reason: 'NativeImpl.cpp modules do not use Swift @_cdecl function stubs');
    });
  ...
  group('multi-spec generate — testing_mixed (Swift/Kotlin/C++ per platform)', skip: multiSpecSkip(), () {
    test('testing_mixed.bridge.g.swift has @_cdecl stubs (iOS is Swift)', () {
      ...
      expect(swift.existsSync(), isTrue);
      expect(swift.readAsStringSync(), contains('@_cdecl("'), reason: 'TestingMixed uses NativeImpl.swift on iOS — Swift @_cdecl stubs required');
    });
    ...
    test('testing_mixed.bridge.g.kt declares TestingMixedJniBridge (Android is Kotlin)', () {
      ...
      expect(kt.existsSync(), isTrue);
      final content = kt.readAsStringSync();
      expect(content, contains('TestingMixedJniBridge'), reason: 'TestingMixed uses NativeImpl.kotlin on Android');
      expect(content, contains('fun platform()'), reason: 'Kotlin interface must declare the platform() method');
    });
    ...
    test('testing_mixed.bridge.g.h exists (macOS/Windows/Linux are C++)', () {
      ...
      expect(h.existsSync(), isTrue);
      final content = h.readAsStringSync();
      expect(content, contains('testing_mixed_platform'), reason: 'C bridge exports testing_mixed_platform for macOS/desktop C++ path');
    });
  ...
  group('multi-spec link — all 3 specs wired into plugin entry points', skip: multiSpecSkip(), () {
    test('iOS Plugin.swift registers all Swift modules (testing_project + testing_mixed)', () {
      ...
      expect(swift.existsSync(), isTrue);
      final content = swift.readAsStringSync();
      expect(content, contains('TestingProjectRegistry.register('), reason: 'TestingProject (Swift) must be registered');
      expect(content, contains('TestingMixedRegistry.register('), reason: 'TestingMixed (Swift on iOS) must be registered');
      expect(content, isNot(contains('TestingCppRegistry')), reason: 'TestingCpp (NativeImpl.cpp) has no Swift registry — no registration needed');
    });
    ...
    test('Android Plugin.kt registers all Kotlin modules (testing_project + testing_mixed)', () {
      ...
      expect(kt.existsSync(), isTrue);
      final content = kt.readAsStringSync();
      expect(content, contains('TestingProjectJniBridge'), reason: 'TestingProject (Kotlin) bridge must be imported/used');
      expect(content, contains('TestingMixedJniBridge'), reason: 'TestingMixed (Kotlin on Android) bridge must be imported/used');
      expect(content, isNot(contains('TestingCppJniBridge')), reason: 'TestingCpp (NativeImpl.cpp) has no JNI bridge — no registration needed');
    });
    ...
    test('iOS Package.swift declares both Swift and C++ SPM targets', () {
      ...
      expect(content, contains('"testing_project"'), reason: 'Swift target named by plugin name for Swift modules');
