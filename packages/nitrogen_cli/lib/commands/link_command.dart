...
void _syncCppModuleSourcesToSpm(
  ...
  for (final platform in ['ios', 'macos']) {
    ...
    //    links the C bridge symbols (testing_project_init_dart_api_dl, etc.)
    //    that are defined in the generated .bridge.g.cpp.
    //    Without this file the symbol is missing at runtime under SPM.
    //    Foundation must be imported before the .cpp because the bridge uses
    //    #ifdef __OBJC__ blocks with NSException / @try-@catch.
    final mainBridgeCpp = File(p.join(
      baseDir, 'lib', 'src', 'generated', 'cpp', '$pluginName.bridge.g.cpp',
    ));
    if (mainBridgeCpp.existsSync()) {
      final relBridge = p
          .relative(mainBridgeCpp.path, from: cppTargetDir.path)
          .replaceAll(r'\', '/');
