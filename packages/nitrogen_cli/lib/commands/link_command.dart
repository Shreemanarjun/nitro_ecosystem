...
class _LinkViewState extends State<LinkView> {
  ...
  List<Map<String, String>> _discoverModuleLibs(String pluginName) => discoverModules(pluginName);

  String _resolveNitroNativePath() => resolveNitroNativePath(p.absolute('.'));

  bool _cmakeNitroNativeExists(String cmakeNitroNative) => nitroNativePathExists(cmakeNitroNative, p.absolute('src'));

  void _createSharedHeaders(String nitroNativePath) {
    Directory('src').createSync(recursive: true);
    final src = File(p.join(nitroNativePath, 'nitro.h'));
    if (src.existsSync()) {
      src.copySync(p.join('src', 'nitro.h'));
      if (Directory('ios/Classes').existsSync()) {
        src.copySync(p.join('ios', 'Classes', 'nitro.h'));
      }
    }

    File(p.join('src', 'dart_api_dl.c')).writeAsStringSync(dartApiDlForwarderContent(nitroNativePath));
  }

  void _linkCMake(String pluginName, List<String> moduleLibs, String nitroNativePath) {
    ...
    _createSharedHeaders(nitroNativePath);
    ...
    if (!content.contains('NITRO_NATIVE')) {
      ...
      if (staleMatch != null) {
        ...
        if (!_cmakeNitroNativeExists(currentVal) && currentVal != desiredNitroValue) {
    ...
    if (!content.contains('NITRO_NATIVE') || !content.contains('packages/nitro/src/native') && !content.contains(nitroNativePath)) {
    ...
    if (content.contains(r'${NITRO_NATIVE}/dart_api_dl.c')) {
      content = content.replaceAll(r'"${NITRO_NATIVE}/dart_api_dl.c"', '"dart_api_dl.c"');
      modified = true;
  ...
  void _generateCMake(String pluginName, List<String> moduleLibs, String nitroNativePath) {
    Directory('src').createSync(recursive: true);

    final String nitroValue;
    if (_cmakeNitroNativeExists(r'${CMAKE_CURRENT_SOURCE_DIR}/../../packages/nitro/src/native')) {
      nitroValue = r'${CMAKE_CURRENT_SOURCE_DIR}/../../packages/nitro/src/native';
    } else {
...
''';
  ...
  void _linkPodspec(String pluginName, List<String> moduleLibs) {
    ...
    const correctSearchPath = r'"${PODS_ROOT}/../.symlinks/plugins/nitro/src/native"';
    if (!content.contains('HEADER_SEARCH_PATHS')) {
      ...
      modified = true;
    } else if (content.contains('PODS_TARGET_SRCROOT') && content.contains('HEADER_SEARCH_PATHS') && !content.contains(correctSearchPath)) {
      // Migrate from the old (wrong) path to the correct one.
      content = content.replaceFirst(
        RegExp(r'''\$\{PODS_TARGET_SRCROOT\}\/\.\.\/(\.\.\/)*packages\/nitro\/src\/native'''),
    ...
    const forwarderContent = '// Forwarder — compiled by CocoaPods/SPM so the Dart DL API is\n'
        '// available in the dylib. Kept as .c so it compiles as C, not C++.\n'
        '#include "../../src/dart_api_dl.c"\n';
    if (!dartApiDlC.existsSync() || dartApiDlC.readAsStringSync().contains('packages/nitro/src/native')) {
    ...
    if (generatedDir.existsSync()) {
      for (final platform in ['swift', 'cpp']) {
        ...
        for (final file in platformDir.listSync().whereType<File>()) {
          ...
          final targetName = fileName.endsWith('.bridge.g.cpp') ? fileName.replaceFirst('.bridge.g.cpp', '.bridge.g.mm') : fileName;
    ...
    // Ensure the main plugin .cpp entry exists (for Xcode and CMake).
    final mainCpp = File(p.join(classesDir.path, '$pluginName.cpp'));
    const mainCppContent = '// Generated by nitrogen link — do not edit.\n'
        '// The bridge code is compiled directly by CocoaPods (Classes/**/*).\n';
    if (!mainCpp.existsSync() || mainCpp.readAsStringSync().contains('.bridge.g.cpp')) {
...
''');
  ...
  void _linkKotlinPlugin(String pluginName, List<Map<String, String>> modules) {
    ...
    // ── System.loadLibrary ───────────────────────────────────────────────
    final missingLibs = moduleLibs.where((l) => !content.contains('System.loadLibrary("$l")')).toList();
    if (missingLibs.isNotEmpty) {
      if (!content.contains('System.loadLibrary')) {
        final className = p.basenameWithoutExtension(pluginFile.path);
        final loadLines = moduleLibs.map((l) => '      System.loadLibrary("$l")').join('\n');
        content = content.replaceFirst(
