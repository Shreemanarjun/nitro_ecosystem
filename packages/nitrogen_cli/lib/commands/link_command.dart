...
List<Map<String, String>> discoverModules(String pluginName) {
  ...
  for (final spec in specs) {
    ...
    final moduleMatch = RegExp(r'abstract class (\w+) extends HybridObject')
        .firstMatch(content);
...
class _LinkViewState extends State<LinkView> {
  ...
  Future<void> _run() async {
    ...
    _linkCMake(
        pluginName, moduleLibs.map((m) => m['lib']!).toList(), nitroNativePath);
    await _setDone(1);
...
''';
  ...
  void _linkPodspec(String pluginName, List<String> moduleLibs) {
    ...
    if (!mainCpp.existsSync() ||
        mainCpp.readAsStringSync().contains('.bridge.g.cpp')) {
  ...
  void _linkSwiftPlugin(String pluginName, List<Map<String, String>> modules) {
    ...
    for (final module in modules) {
      ...
      final implName =
          name.endsWith('Module') ? '${name}Impl' : '${name}ModuleImpl';
      ...
      if (!content.contains('$registry.register')) {
        ...
        final registryBlockMatch =
            RegExp(r'\w+Registry\.register\(.*?\)\)').allMatches(content);
        if (registryBlockMatch.isNotEmpty) {
          ...
          final funcMatch = RegExp(
                  r'public static func register\(with registrar: FlutterPluginRegistrar\) \{')
              .firstMatch(content);
          if (funcMatch != null) {
            content = content.replaceFirst(
              funcMatch.group(0)!,
  ...
  void _linkKotlinPlugin(String pluginName, List<Map<String, String>> modules) {
    ...
    for (final module in modules) {
      ...
      if (!content.contains('$bridge.register')) {
        final bridgeBlockMatch =
            RegExp(r'\w+JniBridge\.register\(.*?\)\)').allMatches(content);
        if (bridgeBlockMatch.isNotEmpty) {
          ...
          final funcMatch = RegExp(r'override fun onAttachedToEngine\(.*?\) \{')
              .firstMatch(content);
          if (funcMatch != null) {
            content = content.replaceFirst(
              funcMatch.group(0)!,
