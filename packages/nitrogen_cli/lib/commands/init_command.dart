...
class _InitViewState extends State<InitView> {
  ...
  void _writeApplePackageSwift(String path, String pluginName, String className, String platformSpec) {
    ...
    final dartApiDlSource = isIos
        ? '../../../.symlinks/plugins/nitro/src/native/dart_api_dl.c'
        : '../../../Flutter/ephemeral/.symlinks/plugins/nitro/src/native/dart_api_dl.c';

    File(p.join(cppSrcDir.path, '$pluginName.cpp')).writeAsStringSync(
    ...
    File(p.join(cppSrcDir.path, 'dart_api_dl.c')).writeAsStringSync(
      ...
      '#include "$dartApiDlSource"\n',
    );

    // Main plugin bridge forwarder — compiled as Obj-C++ (.mm) so SPM links
    // the C symbols defined in the generated .bridge.g.cpp (e.g.
    // <plugin>_init_dart_api_dl).  Without this file the app crashes at
    // startup with: Failed to lookup symbol '<plugin>_init_dart_api_dl'.
    //
    // The nested SPM layout puts this file 4 levels below the plugin root:
    //   <platform>/<plugin>/Sources/<Plugin>Cpp/
    // so the generated file is always at ../../../../lib/src/generated/cpp/.
    //
    // nitrogen link rewrites this file with the resolved absolute path;
    // the relative path written here is a working placeholder that ensures
    // the forwarder exists even before nitrogen link is run.
    File(p.join(cppSrcDir.path, '$pluginName.bridge.g.mm')).writeAsStringSync(
      '// Generated by nitrogen init — do not edit.\n'
