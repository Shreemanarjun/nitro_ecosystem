...
class _InitViewState extends State<InitView> {
  ...
  static void _configureAndroid(String pluginName, String className, String org) {
    ...
    // The plugin class is USER-OWNED once it exists (developers add imports,
    // activity wiring, texture registration, …) — an unconditional rewrite
    // silently dropped those edits on every re-run (issue #16). Same guard
    // the Impl.kt starter below has always had; `nitrogen link` keeps the
    // managed import/register lines up to date additively.
    final pluginFile = File(p.join(kotlinDir.path, '${className}Plugin.kt'));
    if (!pluginFile.existsSync()) {
      pluginFile.writeAsStringSync(androidPluginKtTemplate(org, pluginName, className, moduleName));
    }
