...
- **Fixed: the per-module SPM exports header no longer breaks Apple builds** ([#21](https://github.com/Shreemanarjun/nitro_ecosystem/issues/21)) — 0.5.13 emitted the module target's Dart-DL re-export header as `<TargetName>.h`, which SwiftPM promotes to THE umbrella header; that layout forbids sibling directories inside the public headers dir (e.g. `include/internal/`), so package resolution failed with "invalid header layout" before compiling anything. The header is now `<TargetName>Exports.h` — any non-target name falls back to SPM's permissive umbrella-directory module map, where subdirectories are fine. `nitrogen link` also repairs affected layouts: the 0.5.13 umbrella file is deleted unconditionally (SPM rejects the whole package while it exists), and dart DL header copies (plus `internal/`) hand-placed into module include dirs by pre-0.5.13 workarounds are removed — but ONLY when the module target declares a dependency on the plugin-level Cpp target, so the headers resolve through it. A hand-authored target without that dependency (nitro_webgpu's real layout) keeps its load-bearing copies untouched. The module's own `.bridge.g.h` and any user-added headers are never removed. Verified with `flutter build macos` on a module include dir containing a sibling subdirectory — the exact shape 0.5.13 failed on.
- **Fixed: still-used all-cpp `JniBridge` imports survive `nitrogen link`** ([#16](https://github.com/Shreemanarjun/nitro_ecosystem/issues/16), reopened) — the stale-registration purge for Android-C++ modules removed the module's `import nitro.<lib>_module.<Module>JniBridge` unconditionally, breaking `compileDebugKotlin` when the class was still used (an ActivityAware plugin calling `onActivityAttached` etc. — all-cpp modules still emit a JniBridge with lifecycle hooks). The import is now removed only when NOTHING else in the file references the class (identifier-boundary check; package-qualified references don't count — they compile without the import). The purge patterns are also line-anchored and boundary-checked so a module whose class name is a suffix of a sibling's (`Present` vs `WebgpuPresent`) can never corrupt or mis-classify the sibling's lines.
