...
- **Ecosystem sync** — Also aligned with the 0.5.6 JNI global-reference leak fix on Android zero-copy stream events (entirely in `nitro_generator`'s generated C++ bridge — see its changelog, and regenerate your plugin to pick up both fixes).
