...
**Bridge memory-model release.** Sync bridge returns no longer heap-allocate: the
generated native code hands Dart a pointer into a reusable per-thread slot, and
Dart no longer frees it. `@nitroAsync` / `@nitroNativeAsync` are unchanged — they
decode on a different isolate thread (via `Isolate.run`), where a per-thread slot
is different storage, so those keep their own heap allocation and free.

> **Regenerate after upgrading.** The Dart↔native ownership contract changed in
> lockstep. Code generated by `nitro_generator` 0.6.0 requires `nitro` ≥ 0.6.0
> (it calls the new `toDartStringBorrowed()`); run `nitrogen generate && nitrogen link`.

- **Nullable primitives (`int?` / `double?` / `bool?` / `DateTime?` / `uint64?`) — no malloc per call.** Marshalling drops from **15.03 ns → 0.22 ns** per call (~67×), and **3.61 ns → 0.06 ns** under 4-thread allocator contention. Bench: `benchmark/tool/bench_opt_return.cpp`.
- **`@HybridStruct` returns — no shell malloc.** **16.09 ns → 0.99 ns** (~16×); 4-thread **4.29 → 0.25 ns**. Inner heap fields (strings, nested structs) are still owned and released via `freeFields`. Bench: `bench_struct_return.cpp`.
- **`String` returns — no `strdup` per call.** A per-thread `std::string` reuses its capacity, so a warm thread allocates nothing: **27.8 ns → 9.9 ns** for a 32-byte string (~2.8×), **30.9 → 10.1 ns** at 128 bytes. New `toDartStringBorrowed()` decodes without freeing. Bench: `bench_string_return.cpp`.
- **Multi-instance dispatch — 8-way instance cache.** The C-bridge instance lookup kept a single `(id, ptr)` entry, so any loop rotating across more than one instance fell back to a mutex + hashmap on *every* call. It is now 8 `alignas(64)` slots striped by id: **7–8× faster** with 2/4/8 live instances and **48× under 4-thread contention** (the old path serialized readers on the mutex). Single-instance dispatch is unchanged. Bench: `bench_instance_cache.cpp`.
...
- **Cross-platform verification.** Validated on macOS (desktop C++), the iOS simulator (Swift) and an Android emulator (JNI/Kotlin) through `nitro_type_coverage`: 679 integration tests per Apple platform, plus 4342 generator tests. Three bugs were caught only because all three platforms were exercised — Apple's allocator silently tolerated a bad free that Android's Scudo aborted on.
