...
class SpecValidator {
  static const _knownPrimitives = {
    'int', 'double', 'bool', 'String', 'void', 'Uint8List',
    'int?', 'double?', 'bool?', 'String?', 'Uint8List?',
  };
  ...
  static List<ValidationIssue> validate(BridgeSpec spec) {
    ...
    for (final func in spec.functions) {
      ...
      if (retName != 'void' && !knownTypes.contains(retName) && !knownTypes.contains(func.returnType.name)) {
        issues.add(ValidationIssue(
          severity: ValidationSeverity.error,
          code: 'UNKNOWN_RETURN_TYPE',
          message: '${spec.dartClassName}.${func.dartName}() — unknown return type "$retName".',
          hint: 'If "$retName" is a struct, annotate it with @HybridStruct. '
      ...
      for (final param in func.params) {
        ...
        if (!knownTypes.contains(pName) && !knownTypes.contains(param.type.name)) {
          issues.add(ValidationIssue(
            severity: ValidationSeverity.error,
            code: 'UNKNOWN_PARAM_TYPE',
            message: '${spec.dartClassName}.${func.dartName}() — parameter "${param.name}" has unknown type "$pName".',
            hint: 'If "$pName" is a struct, annotate it with @HybridStruct. '
                  'If it is an enum, annotate it with @HybridEnum.',
          ));
        }
      }
    ...
    for (final stream in spec.streams) {
      ...
      if (!knownTypes.contains(iName) && !knownTypes.contains(stream.itemType.name)) {
        issues.add(ValidationIssue(
          severity: ValidationSeverity.error,
          code: 'UNKNOWN_STREAM_ITEM_TYPE',
          message: '${spec.dartClassName}.${stream.dartName} — unknown stream item type "$iName".',
          hint: 'Stream item types must be primitives, String, Uint8List, or a @HybridStruct. '
      ...
      for (final field in st.fields) {
        ...
        if (!_knownPrimitives.contains(fName) && !structNames.contains(fName)) {
          issues.add(ValidationIssue(
            ...
            hint: 'Struct fields must be int, double, bool, String, Uint8List, or another @HybridStruct.',
          ));
        }
        ...
        if (field.zeroCopy && fName != 'Uint8List') {
          issues.add(ValidationIssue(
            ...
            message: '${st.name}.${field.name} — @zero_copy is only valid on Uint8List fields (got "$fName").',
          ));
        }
      }
    }
