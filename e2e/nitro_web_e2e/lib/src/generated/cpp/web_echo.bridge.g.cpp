...
EM_JS(int, nitro_web_instance_changed, (), {
    ...
    if (reg["web_echo"] === wasmExports) return 0;
    reg["web_echo"] = wasmExports;
    return 1;
});
...
void _nitro_release_instance_streams(const void* impl) {
    g_ports_ticks.dropInstance(impl);
}

void HybridWebEcho::emit_ticks(int64_t item) {
    auto _ports = g_ports_ticks.snapshot(this);
...
#ifndef __EMSCRIPTEN__
    g_nitro_batch_web_echo.coalesce(dart_port);
#endif
    g_ports_ticks.add(_nitro_get_instance(instanceId), dart_port);
}
void web_echo_release_ticks_stream(int64_t dart_port) {
    g_ports_ticks.remove(dart_port);
#ifndef __EMSCRIPTEN__
    g_nitro_batch_web_echo.uncoalesce(dart_port);
#endif
}
